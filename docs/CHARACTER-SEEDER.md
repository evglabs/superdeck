...
| `--database` | No | `superdeck.db` | SQLite database path |
...
Verbose output shows full character details:

```bash
dotnet run --project src/Tools/SuperDeck.Tools.CharacterSeeder -- \
  --name Shadow --suit Espionage --verbose --database src/Server/superdeck.db
```
